...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='/Users/yinhao/PycharmProjects/semseg/config/ade20k/ade20k_pspnet101.yaml', help='config file')
...
def seg(model, image_seg, classes, mean, std, base_size, crop_h, crop_w, scales, colors):
    # image = cv2.imread(image_path, cv2.IMREAD_COLOR)  # BGR 3 channel ndarray wiht shape H * W * 3
    image = cv2.cvtColor(image_seg, cv2.COLOR_BGR2RGB)  # convert cv2 read image from BGR order to RGB order
    h, w, _ = image.shape
    ...
    # gray_path = os.path.join('/Users/yinhao/PycharmProjects/semseg/figure/demo/', image_name + '_gray.png')
    # color_path = os.path.join('/Users/yinhao/PycharmProjects/semseg/figure/demo/', image_name + '_color.png')
...
if __name__ == '__main__':
    t1 = time.time()
    cfg = get_parser()
    imagedir = "/Users/yinhao/PycharmProjects/semseg/figure/demo/"
    savedir = "/Users/yinhao/PycharmProjects/semseg/figure/results/"
    model, mean, std, colors = eval()
    num = 0
    for photo_name in os.listdir(imagedir):
        num += 1
        t = time.time()
        img_path = os.path.join(imagedir+os.sep, photo_name)
        color = seg(model.eval(), img_path, cfg.classes, mean, std, cfg.base_size, cfg.test_h, cfg.test_w, cfg.scales, colors)
        print(color)

        if color is not None:
            color.save(savedir+"/"+"{}.png".format(photo_name.split(".")[0]))
